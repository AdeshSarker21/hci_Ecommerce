<x-seller.layout title="Add Staff Member" active="staff">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Add Staff Member</h2>
            <p class="text-sm text-gray-500 mt-1">Invite a new team member to your store</p>
        </div>
    ...
    <form method="POST" action="{{ route('seller.staff.store') }}">
        @csrf
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="mt-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('can_manage_products') ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200' }}">
                        <input type="checkbox" name="can_manage_products" value="1"
                               {{ old('can_manage_products') ? 'checked' : '' }}
                               class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                        <span class="ml-2 text-sm text-gray-700">Manage Products</span>
                    </label>
                    <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('can_manage_orders') ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200' }}">
                    ...
                    <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('can_manage_settings') ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200' }}">
                        <input type="checkbox" name="can_manage_settings" value="1"
                               {{ old('can_manage_settings') ? 'checked' : '' }}
                               class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                        <span class="ml-2 text-sm text-gray-700">Manage Settings</span>
                    </label>
                    <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('can_view_reports') ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200' }}">
            ...
            <div class="mt-4">
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm"
                          placeholder="Internal notes about this staff member...">{{ old('notes') }}</textarea>
            </div>
        </div>
